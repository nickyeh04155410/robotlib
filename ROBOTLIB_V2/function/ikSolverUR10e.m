function q = ikSolverUR10e(pose, qPrevious, alpha, a, d, TCP, offset)    
    %The input tform is a 4x4 matrix with homogeneus trasform matrix of the end-effector.
    ...
    T_tcp2base = vec2homogeneous(pose);
    T_tcp26 = vec2homogeneous(TCP);
    T_02base = vec2homogeneous(offset);
    
    % Solve theta1
    T06 = (T_02base \ T_tcp2base) / T_tcp26;
    P06 = reshape(T06(1:3, 4), [3, 1]);
...
function solution = closetSolution(solutions, q)
    % Step 1: 選擇與 theta1 最近的解
    [~, idx_theta1] = min(abs(solutions(:, 1) - q(1)));
    solutions = solutions(solutions(:, 1) == solutions(idx_theta1, 1), :);

    % Step 2: 在剩餘的解中，選擇與 theta5 最近的解
    [~, idx_theta5] = min(abs(solutions(:, 5) - q(5)));
    solutions = solutions(solutions(:, 5) == solutions(idx_theta5, 5), :);

    % Step 3: 在剩餘的解中，選擇與 theta3 最近的解
    [~, idx_theta3] = min(abs(solutions(:, 3) - q(3)));
    solutions = solutions(solutions(:, 3) == solutions(idx_theta3, 3), :);

    % Step 4: 如果還有多個解，根據加權距離選擇最佳解
    if size(solutions, 1) > 1
        weights = [6 5 4 3 2 1];
        distances = sum(((solutions - q') .* weights).^2, 2);
        [~, idx] = min(distances);
